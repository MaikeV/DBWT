...
class RegistrationController {
    function getRegister() {
        ...
        if ($resultFields = mysqli_query($this->database, $queryFields)) {
        ...
        return view("pages.Registration", ["registered" => $registered, "firstName" => $firstName, "lastName" => $lastName,
                            "birthday" => $birthday, "email" => $email, "password" => $password, "role" => $role, "username" => $username,
                            "option1" => $option1, "option2" => $option2, "message" => $message, "studies" => $studies, "phone" => $phone,
                            "matrikel" => $matrikel, "room" => $room,"reason" => $reason, "endDate" => $endDate, "fields" => $fields,
                            "fieldNum" => $fieldNum, "message" => $message, "errors" => $errors, "remoteConnection" => $remoteConnection,
                            "loggedIn" => $loggedIn]);
    }
...
//echo $blade->run("layouts.footer", array("year" => $year));
    ...
    function register($username, $email, $password, $firstName, $lastName, $birthday, $role, $option1, $option2, &$errors, &$message, $fieldNum, $remoteConnection) {
        if ($this->checkUserExists($username, $errors, $remoteConnection) || $this->checkEmailExists($email, $errors, $remoteConnection)) {
        ...
        if(mysqli_query($remoteConnection, $queryUser)) {
            if($role == "Student") {
                ...
                $this->addToField($fieldNum, $remoteConnection);
            } else if($role == "Mitarbeiter") {
...
//            session()->start();
            ...
            $userNum = $this->getUsernum($username, $remoteConnection);
