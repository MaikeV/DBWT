...
<body class="bg-dark">
    <?php
        ...
        if (isset($_GET['id'])) {
            $query = "SELECT * FROM Mahlzeiten WHERE ID LIKE ".$_GET['id'];
            $count = "SELECT COUNT(*) count FROM Mahlzeiten";

            if ($rows = mysqli_query($remoteConnection, $count)) {
        ...
        function redirToMahlzeiten($remoteConnection) {
            ...
            sleep(3);
            header("Location: http://127.0.0.1/M2/static/Mahlzeiten.php");
                ...
                <div class="col-3 p-5">
                    <div class="row">
                        <div class="col-12 text-center text-warning">
                            ...
                            <h4>
                                <?php
                                    $priceQuery = "SELECT ROUND(Gastpreis, 2) AS Gastpreis FROM Preise WHERE ID LIKE ".$_GET['id'];

                                    $price = mysqli_query($remoteConnection, $priceQuery);
                                    $priceRow = mysqli_fetch_assoc($price);

                                    echo $priceRow['Gastpreis'].' EUR';
                                ?>
                            </h4>
                        </div>
                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-9">
                                            <select class="form-control w-50" id="dishInput" name="mahlzeit" required>
                                                <option value="">---</option>
                                                <?php
                                                    $mealsQuery = "SELECT Name FROM Mahlzeiten";

                                                    if ($mealsResult = mysqli_query($remoteConnection, $mealsQuery)) {
                                                        while($mealsRow = mysqli_fetch_assoc($mealsResult)) {
                                                            echo '<option>'.$mealsRow['Name'].'</option>';
                                                        }
                                                    }
                                                ?>
                                            </select>
